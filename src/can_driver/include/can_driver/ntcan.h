/* ntcan.h
**
**            Copyright (c) 2001 - 2017 by esd electronics gmbh
**
...
#define NTCAN_MODE_FD                   0x00040000  /* Enable CAN-FD support on handle            */
...
#if defined(NTCAN_FD)
        #error "The NTCAN_LEN_TO_DATASIZE() and NTCAN_DATASIZE_TO_DLC() must be changed for NTCAN_FD operation!"
#else
        #define NTCAN_LEN_TO_DATASIZE(len)      (((len) & 0x08) ? 8 : ((len) & 0x07))
        #define NTCAN_DATASIZE_TO_DLC(ds)       NTCAN_DLC(ds)
#endif

/*
 * Macros for parameter id of CMSG / CMSG_T structure
 */
...
typedef struct {
        ...
        const uint16_t open_handle;        /* Number of open handles                */
        const uint8_t  reserved2[42];      /* Reserved for future use               */
} NTCAN_INFO;
